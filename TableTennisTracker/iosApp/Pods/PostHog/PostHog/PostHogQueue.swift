...
private
    let pausedLock = NSLock()
private
    var pausedUntil: Date ?
private
    var retryCount: TimeInterval = 0
#if !os(watchOS)
...
private
    let isFlushingLock = NSLock()
private
    var timer: Timer ?
private
    let timerLock = NSLock()
private
    let endpoint: PostHogApiEndpoint
private
...
#else
            init(_
    config: PostHogConfig, _
    storage: PostHogStorage, _
    api: PostHogApi, _
...
private
    ...
    : PostHogConsumerPayload) {
        ...
        switch endpoint{
                    case .batch:
                    api.batch(events: payload.events) {
                        result in
                        self.handleResult(result, payload)
                    }
                    case .snapshot:
                    api.snapshot(events: payload.events) {
                        result in
                        self.handleResult(result, payload)
                    }
            }
...
private
    ...
    : PostHogBatchUploadInfo,
    _ payload: PostHogConsumerPayload
    ) {
    ...
    func start(disableReachabilityForTesting
                    ...
                    timerLock.withLock {
                        DispatchQueue.main.async
                        {
                            self.timer = Timer.scheduledTimer(
                                    withTimeInterval: self.config.flushIntervalSeconds, repeats: true, block: {
                                _ in
                                if !self.isFlushing
                                {
                                    self.flush()
...
private
    ...
    : PostHogEvent) {
        ...
        do {
            data =
            try
            JSONSerialization.data(withJSONObject: event.toJSON())
        } catch {
...
private
    ...
    completion:
    @
    escaping (PostHogConsumerPayload)
    -> Void) {
        dispatchQueue.async
        {
            ...
            completion(PostHogConsumerPayload(events: processing) {
                success in
