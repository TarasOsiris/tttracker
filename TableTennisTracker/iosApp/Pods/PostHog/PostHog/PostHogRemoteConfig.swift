...
private
    let hasFeatureFlagsKey = "hasFeatureFlags"
...
private
    let api: PostHogApi
private
    let getDefaultPersonProperties: ()->[String :Any]

private
    let loadingFeatureFlagsLock = NSLock()
private
    let featureFlagsLock = NSLock()
private
    var loadingFeatureFlags = false
private
    var sessionReplayFlagActive = false
...
private
    var remoteConfigLock = NSLock()
private
    let loadingRemoteConfigLock = NSLock()
private
    var loadingRemoteConfig = false
private
    var remoteConfig: [String :Any] ?
private
    var remoteConfigDidFetch: Bool = false
private
    var featureFlagPayloads: [String :Any] ?
private
...
private
    let personPropertiesForFlagsLock = NSLock()
private
    var personPropertiesForFlags: [String :Any] = [:]

private
    let groupPropertiesForFlagsLock = NSLock()
private
    var groupPropertiesForFlags: [String :[String :Any]] = [:]

    /// Internal, only used for testing
    var canReloadFlagsForTesting = true

    var onRemoteConfigLoaded: (([String :Any] ?)->Void) ?
                              var onFeatureFlagsLoaded
    : (([
    String: Any
    ]?) -> Void)?
...
private
    ...
    api: PostHogApi,
            _
    getDefaultPersonProperties: @escaping ()->
    [
    String: Any
    ])
    {
                                     ...
                                     })
        {
            dispatchQueue.async
            {
                self.reloadRemoteConfig
                {
                    ...
                    if let
                    hasFeatureFlagsBoolValue = remoteConfig[self.hasFeatureFlagsKey]
                    as ? Bool, !hasFeatureFlagsBoolValue{
                            hedgeLog(
                                    "hasFeatureFlags is false, clearing flags and skipping loading flags")
...
private
    ...
    func preloadSessionReplayFlag() {
        var
        sessionReplay:
        [String :Any] ?
        var featureFlags: [String :Any] ?
                          featureFlagsLock.withLock{
                                  sessionReplay = self.storage.getDictionary(
                                          forKey: .sessionReplay) as?[String: Any]
...
private
    ...
    func isRecordingActive(_ featureFlags

    : [
    String: Any
    ],
    _ sessionRecording: [String :Any]
    ) -> Bool {
        var recordingActive = true

        // check for boolean flags
        if let
        linkedFlag = sessionRecording["linkedFlag"]
        as ? String{
    ...
    anonymousId: String ?,
                ...
                groupProperties: groupProperties.isEmpty ? nil : groupProperties)
        {
            ...
            self.dispatchQueue.async
            {
                            ...
                            quotaLimited.contains("feature_flags")
                                                              {
                                                                      ...
                                                                      hedgeLog(
                                                                              "Warning: Feature flags quota limit reached - flags could not be updated. See https://posthog.com/docs/billing/limits-alerts for more information.")
                ...
                as ? [String :Any],
                        let featureFlagPayloads = data["featureFlagPayloads"]
                as ? [String :Any]
                else {
                ...
                as ? Int
                let
                errorsWhileComputingFlags = data["errorsWhileComputingFlags"]
                as ? Bool ?? false
                var
                loadedFeatureFlags:
                [String :Any] ?
                ...
                self.featureFlagsLock.withLock{
                        ...
                        if errorsWhileComputingFlags {
                            ...
                            let
                            cachedFlags = self.getCachedFlags() ?? [:]
                            ...
                            let
                            cachedFeatureFlags = self.getCachedFeatureFlags() ?? [:]
                            let
                            cachedFeatureFlagsPayloads = self.getCachedFeatureFlagPayload() ?? [:]

                            let newFeatureFlags = cachedFeatureFlags.merging(featureFlags) {
                                _, new in
                                new
                            }
                            let newFeatureFlagsPayloads = cachedFeatureFlagsPayloads.merging(
                                    featureFlagPayloads) {
                                _, new in
                                new
                            }
                            ...
                            if let flagsV4{
                                        let newFlags = cachedFlags.merging(flagsV4) {
                                            _, new in
                                            new
                                        }
                                        // if not all flags were computed, we upsert flags instead of replacing them
...
private
    ...
    func notifyFeatureFlags(_ featureFlags
...
private
    ...
    func notifyFeatureFlagsAndRelease(_ featureFlags
    ...
    func getFeatureFlagDetails(_ key
...
private
    ...
    func getCachedFeatureFlagPayload()

    -> [
    String: Any
    ]? {
        if featureFlagPayloads == nil{
                    featureFlagPayloads = storage.getDictionary(
                            forKey: .enabledFeatureFlagPayloads) as?[String: Any]
            }
...
private
    ...
    func setCachedFeatureFlagPayload(_ featureFlagPayloads
...
private
    ...
    func getCachedFeatureFlags()

    -> [
    String: Any
    ]? {
        if featureFlags == nil{
                    featureFlags = storage.getDictionary(
                            forKey: .enabledFeatureFlags) as?[String: Any]
            }
        return featureFlags
    }
...
private
    ...
    func setCachedFeatureFlags(_ featureFlags
...
private
    ...
    String: Any
        ...
        personPropertiesForFlagsLock.withLock
        {
            ...
            personPropertiesForFlags.merge(properties, uniquingKeysWith: {
                _, new in
                new
            })
            // Persist to disk
            storage.setDictionary(
                    forKey: .personPropertiesForFlags, contents: personPropertiesForFlags)
        }
    }
    ...
    properties: [String :Any]
        ...
        groupPropertiesForFlagsLock.withLock
        {
            ...
            [groupType, default: [:]].merge(properties)
            {
                _, new in
                new
            }
            // Persist to disk
            storage.setDictionary(
                    forKey: .groupPropertiesForFlags, contents: groupPropertiesForFlags)
        }
    }
...
private
    ...
    func getGroupPropertiesForFlags()
...
private
    ...
    func getPersonPropertiesForFlags()
    ...
    String: Any
        ...
        personPropertiesForFlagsLock.withLock
        {
            ...
            if config.setDefaultPersonProperties
            {
                ...
                properties = defaultProperties.merging(properties)
                {
                    _, userValue
                    in
                    userValue
...
private
    ...
    func loadCachedPropertiesForFlags() {
        personPropertiesForFlagsLock.withLock
        {
            if let
            cachedPersonProperties = storage.getDictionary(forKey: .personPropertiesForFlags) as
                                                                                              ? [String :Any] {
        ...
        {
            if let
            cachedGroupProperties = storage.getDictionary(forKey: .groupPropertiesForFlags) as
                                                                                            ? [String :[String :Any]] {
    ...
    : String) -> Any? {
        ...
        do {
            ...
            return
            try
            JSONSerialization.jsonObject(with: stringValue.data(
            using: .utf8)!, options: .fragmentsAllowed)
        } catch {
...
private
    ...
    func setCachedRequestId(_ value
...
private
    ...
    func setCachedEvaluatedAt(_ value
