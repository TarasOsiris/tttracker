...
private
    let optOutLock = NSLock()
private
    let groupsLock = NSLock()
private
    let flagCallReportedLock = NSLock()
private
    let personPropsLock = NSLock()

private
    var queue: PostHogQueue ?
private
    var replayQueue: PostHogQueue ?
private(set)
    var storage: PostHogStorage ?
                 ...
                 #endif
private
    var flagCallReported: [String :[Any ?]]
    = .

    init()

private(set)
    var remoteConfig: PostHogRemoteConfig ?
private
    var context: PostHogContext ?
private
    static var apiKeys = Set<String>()
private
    var installedIntegrations: [PostHogIntegration] = []
...
public
    ...
    : PostHogConfig) {
        setupLock.withLock
        {
            ...
            {
                let
                optOut = theStorage.getBool(forKey: .optOut)
...
private
    ...
    String: Any
        ...
        guard
        let
        flags = remoteConfig ?.getFeatureFlags()
        as ? [String :Any] else {
            return properties
        }
...
private
    ...
    func buildProperties(distinctId
    ...
    ]
    {
        ...
        if appendSharedProps{
                    ...
                    if dynamicCtx != nil {
                        props = props.merging(dynamicCtx ? ? [:]) {
                            current, _
                            in
                            current
                        }
                    }
                    props = props.merging(localDynamicCtx) {
                        current, _
                        in
                        current
                    }
                    ...
                    if groups != nil {
                        ...
                        as ? [String :String] ?? [:]
                        let
                        mergedGroups = currentGroups.merging(groups ? ? [:]) {
                            current, _
                            in
                            current
                        }
        ...
        let
        sessionId:
        String ? = propSessionId.isNilOrEmpty
        ...
        props = props.merging(properties ? ? [:]) {
            current, _
            in
            current
        }
...
private
    ...
    String: String
        ...
        guard
        let
        groups = storage ?.getDictionary(forKey: .groups) as ? [String :String] else {
            return [:]
        }
        return groups
    }
...
private
    ...
    String: Any
        ...
        guard
        let
        props = storage ?.getDictionary(forKey: .registerProperties) as ? [String :Any] else {
            return [:]
        }
        return props
    }
...
public
    ...
    properties: [String :Any]
        ...
        personPropsLock.withLock
        {
            ...
            let
            mergedProps = props.merging(sanitizedProps
            !) {
                _, new in
                new
            }
...
public
    ...
    @objc(
    identifyWithDistinctId: userProperties
    :)
public
...
public
    ...
    )
    {
        ...
        let
        queue, let
        storageManager = config.storageManager else {
            return
        }
        ...
        else if !hasDifferentDistinctId, !(userProperties ?.isEmpty
                                         ?? true) || !(userPropertiesSetOnce ?.isEmpty ?? true) {
            capture("$set",
                    distinctId: distinctId,
...
private
    ...
    func setPersonPropertiesForFlagsIfNeeded(
            _ userProperties
    ...
    userPropertiesSetOnce: [String :Any] ? = nil
        ...
        let
        sanitizedUserProperties = sanitizeDictionary(userProperties) ?? [:]
        let
        sanitizedUserPropertiesSetOnce = sanitizeDictionary(userPropertiesSetOnce) ?? [:]

        // Combine both types of properties for feature flag evaluation
        let allProperties = sanitizedUserProperties.merging(sanitizedUserPropertiesSetOnce) {
            current, _
            in
            current
        }
...
private
    ...
    func setGroupPropertiesForFlagsIfNeeded(
            type
    ...
    groupProperties: [String :Any] ?
        ...
        let
        sanitizedGroupProperties = sanitizeDictionary(groupProperties) ?? [:]

        guard
        !sanitizedGroupProperties.isEmpty else {
            return
        }
...
private
    ...
    func getDefaultPersonProperties()

    -> [
    String: Any
    ] {
        guard
        config.setDefaultPersonProperties else { return [:] }
        guard
...
public
    ...
    func capture(_ event

    : String) {
        capture(event,
                distinctId: nil, properties: nil, userProperties: nil, userPropertiesSetOnce: nil, groups: nil)
    }

    @objc(
    captureWithEvent: properties
    :)
public
    ...
    {
        capture(event,
                distinctId: nil, properties: properties, userProperties: nil, userPropertiesSetOnce: nil, groups: nil)
    }
...
public
    ...
    {
        capture(event,
                distinctId: nil, properties: properties, userProperties: userProperties, userPropertiesSetOnce: nil, groups: nil)
    }
...
public
    ...
    {
        capture(event,
                distinctId: nil, properties: properties, userProperties: userProperties, userPropertiesSetOnce: userPropertiesSetOnce, groups: nil)
    }
...
public
    ...
    {
        capture(event,
                distinctId: nil, properties: properties, userProperties: userProperties, userPropertiesSetOnce: userPropertiesSetOnce, groups: groups)
    }

    @objc(
    captureWithEvent: distinctId
    :
    properties: userProperties
    :
    userPropertiesSetOnce: groups
    :)
public
...
public
    ...
    )
    {
        ...
        let
        eventDistinctId = distinctId ?? getDistinctId()
        ...
        if userProperties ?.isEmpty == false || userPropertiesSetOnce ?.isEmpty == false || groups
                                                                        ?.isEmpty == false
        {
        ...
        let
        properties = buildProperties(distinctId: eventDistinctId,
...
public
    ...
    properties: [String :Any] ? = nil
        ...
        props = [
                ...
                ].merging(sanitizeDictionary(properties) ? ? [:]) {
            prop, _
            in
            prop
        }
        ...
        guard
        let
        event = buildEvent(event: "$screen", distinctId: distinctId, properties: properties) else {
            return
        }
    ...
    elementsChain: String,
        ...
        let
        event = buildEvent(
                event: "$autocapture", distinctId: distinctId, properties: properties) else {
            return
        }
...
private
    ...
    func sanitizeProperties(_ properties
...
public
    ...
    : String) {
        ...
        let
        event = buildEvent(
                event: "$create_alias", distinctId: distinctId, properties: properties) else {
            return
        }
...
private
    ...
    String: String
        ...
        var mergedGroups: [String :String] ?
                          groupsLock.withLock{
                                  groups = getGroups()
                                  mergedGroups = groups?.merging(newGroups) {
                                      _, new in
                                      new
                                  }
...
private
    ...
    func groupIdentify(type

    : String,
    key: String, groupProperties: [String :Any] ? = nil
    ) {
        if !isEnabled()
        {
            return
        ...
        let
        event = buildEvent(
                event: "$groupidentify", distinctId: distinctId, properties: properties) else {
            return
        }
    ...
    func buildEvent(event eventName

    : String,
    distinctId: String, properties: [String :Any], timestamp: Date = Date()
    ) -> PostHogEvent? {
...
public
    ...
    func setPersonPropertiesForFlags(_ properties
    ...
    @objc(
    setPersonPropertiesForFlagsWithProperties: reloadFeatureFlags
    :)
public
    ...
    reloadFeatureFlags: Bool = true
        ...
        let
        sanitizedProperties = sanitizeDictionary(properties) ?? [:]
        guard
        !sanitizedProperties.isEmpty else { return }
...
public
    ...
    @objc(
    resetPersonPropertiesForFlagsWithReloadFeatureFlags:
    )
public

    func resetPersonPropertiesForFlags(reloadFeatureFlags
...
public
    ...
    func setGroupPropertiesForFlags(_ groupType
...
public
    ...
    : String,
    properties: [String :Any], reloadFeatureFlags: Bool = true
    ) {
        if !isEnabled()
        {
            return
...
public
    ...
    func resetGroupPropertiesForFlags() {
    ...
    /// ```
    @objc(
    resetGroupPropertiesForFlagsWithReloadFeatureFlags:
    )
public

    func resetGroupPropertiesForFlags(reloadFeatureFlags
    ...
    @objc(
    resetGroupPropertiesForFlagsWithGroupType:
    )
public

    func resetGroupPropertiesForFlags(_ groupType
    ...
    ///   - reloadFeatureFlags: Whether to automatically reload feature flags after setting properties
    @objc(
    resetGroupPropertiesForFlagsWithGroupType: reloadFeatureFlags
    :)
public
    ...
    reloadFeatureFlags: Bool = true
        ...
        internalResetGroupPropertiesForFlags(
                groupType: groupType, reloadFeatureFlags: reloadFeatureFlags)
    }

    /// Internal implementation for resetting group properties.
private

    func internalResetGroupPropertiesForFlags(groupType
...
public
    ...
    @objc(
    reloadFeatureFlagsWithCallback:
    )
public

    func reloadFeatureFlags(_ callback
...
public
    ...
    @objc(
    getFeatureFlagWithKey: sendFeatureFlagEvent
    :)
public
...
private
    ...
    : String,
    sendEvent sendFeatureFlagEvent: Bool ? = nil
    ) -> Any? {
        if !isEnabled()
        {
            return nil
        ...
        let
        shouldSendEvent = sendFeatureFlagEvent ?? config.sendFeatureFlagEvent
        if shouldSendEvent{
...
public
    ...
    @objc(
    isFeatureEnabledWithKey: sendFeatureFlagEvent
    :)
public
...
private
    ...
    sendEvent: Bool ? = nil
        ...
        let
        result = getFeatureFlag(key, sendEvent: sendEvent)
        return result
        is
        String ? true : (result
        as ? Bool) ?? false
...
public
    ...
    func getFeatureFlagPayload(_ key
...
private
    ...
    _ rhs: Any ?
        ...
        switch (lhs, rhs) {
            case (nil, nil):
                return true
            case let(boolValue as
                Bool, flagBoolValue
                as
                Bool):
                return boolValue == flagBoolValue
            case let(stringValue as
...
private
    ...
    func reportFeatureFlagCalled(flagKey
    ...
    flagValue: Any ?
        ...
        {
            var
            values = flagCallReported[flagKey] ?? []
        ...
        if shouldCapture{
                    ...
                    if let details = details as?[String: Any] {
                        ...
                        as ? [String :Any] {
                                properties["$feature_flag_reason"] = reason["description"]
                                                                     ?? NSNull()
                        }
...
private
    ...
    -> Bool {
        if !enabled{
                    hedgeLog(
                            "PostHog method was called without `setup` being complete. Call wil be ignored.")
            }
        return enabled
    }
...
public
    ...
    static func with(_ config

    : PostHogConfig) -> PostHogSDK {
...
private
    ...
    func installIntegrations() {
        guard
        installedIntegrations.isEmpty else {
        ...
        in integrations{
                ...
                if integration.requiresSwizzling, !config.enableSwizzling {
                    hedgeLog(
                            "Integration \(type(of: integration)) skipped. Integration requires swizzling but enableSwizzling is disabled in config")
                    continue
                }
...
private
    ...
    func uninstallIntegrations() {
        for
        integration
        in installedIntegrations{
