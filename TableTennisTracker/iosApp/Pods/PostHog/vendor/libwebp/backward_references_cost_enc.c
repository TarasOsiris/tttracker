...
static WEBP_INLINE int64_t

GetLiteralCost(const CostModel *const m,
               uint32_t v) {
    return (int64_t)
    m->alpha_[v >> 24] + m->red_[(v >> 16) & 0xff] +
    m->literal_[(v >> 8) & 0xff] + m->blue_[v & 0xff];
}

static WEBP_INLINE int64_t

GetCacheCost(const CostModel *const m,
             uint32_t idx) {
    const int literal_idx = VALUES_IN_BYTE + NUM_LENGTH_CODES + idx;
    return (int64_t)
    m->literal_[literal_idx];
}

static WEBP_INLINE int64_t

GetLengthCost(const CostModel *const m,
              uint32_t length) {
    int code, extra_bits;
    VP8LPrefixEncodeBits(length, &code, &extra_bits);
    return (int64_t)
    m->literal_[VALUES_IN_BYTE + code] +
    ((int64_t)
    extra_bits << LOG_2_PRECISION_BITS);
}

static WEBP_INLINE int64_t

GetDistanceCost(const CostModel *const m,
                uint32_t distance) {
    int code, extra_bits;
...
static int CostManagerInit(CostManager *const manager,
    ...
    manager->costs_ =
            (int64_t * )
    WebPSafeMalloc(pix_count, sizeof(*manager->costs_));
