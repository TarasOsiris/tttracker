...
BitsEntropyRefine(const VP8LBitEntropy *entropy) {
    uint64_t mix;
    if (entropy->nonzeros < 5) {
        ...
        if (entropy->nonzeros == 2) {
            return DivRound(99 * ((uint64_t)
            entropy->sum << LOG_2_PRECISION_BITS) +
                    entropy->entropy,
                    100);
        }
...
static uint64_t FinalHuffmanCost(const VP8LStreaks *const stats) {
    ...
    return retval + ((uint64_t)
    retval_extra << (LOG_2_PRECISION_BITS - 10));
}
...
GetCombinedEntropy(const uint32_t *const X,
...
WEBP_NODISCARD static int GetCombinedHistogramEntropy(
    ...
    const uint64_t cost_threshold = (uint64_t)
    cost_threshold_in;
    ...
    *cost += (uint64_t)
    VP8LExtraCostCombined(a->literal_ + NUM_LITERAL_CODES,
    ...
    *cost += (uint64_t)
    VP8LExtraCostCombined(a->distance_, b->distance_,
                          NUM_DISTANCE_CODES)
            << LOG_2_PRECISION_BITS;
    if (*cost >= cost_threshold) return 0;
