...
PixOrCopyLiteral(const PixOrCopy *const p,
                 int component) {
...
PixOrCopyLength(const PixOrCopy *const p) {
    return p->len;
}

static WEBP_INLINE uint32_t

PixOrCopyCacheIdx(const PixOrCopy *const p) {
...
PixOrCopyDistance(const PixOrCopy *const p) {
    ASSERT(p->mode == kCopy);
    return p->argb_or_distance;
}
