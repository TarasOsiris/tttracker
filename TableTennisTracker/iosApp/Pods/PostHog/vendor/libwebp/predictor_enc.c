...
*
GetHistoArgb(uint32_t
* const all_histos,
int subsampling_index,
int mode
) {
return &all_histos[(
subsampling_index *kNumPredModes
+ mode) * HISTO_SIZE];
}

static WEBP_INLINE const uint32_t
*
...
int VP8LResidualImage(int width, int height, int min_bits, int max_bits,
    ...
    if (low_effort) {
        ...
        *best_bits = max_bits;
        ...
        modes_raw = (uint32_t * )
        WebPSafeMalloc(sum_num_pixels, sizeof(*modes_raw));
