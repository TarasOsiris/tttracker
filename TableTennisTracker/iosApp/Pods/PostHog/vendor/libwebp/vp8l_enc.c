...
static int AnalyzeEntropy(const uint32_t *argb,
    ...
    histo = (uint32_t * )
    WebPSafeCalloc(kHistoTotal, sizeof(*histo) * 256);
    if (histo != NULL) {
        int i, x, y;
...
static int WriteRiffHeader(const WebPPicture *const pic, size_t riff_size,
    ...
    PutLE32(riff + TAG_SIZE, (uint32_t)
    riff_size);
    PutLE32(riff + RIFF_HEADER_SIZE + TAG_SIZE, (uint32_t)
    vp8l_size);
...
static int AllocateTransformBuffer(VP8LEncoder *const enc, int width,
    ...
    if (mem == NULL || mem_size > enc->transform_mem_size_) {
        ...
        mem = (uint32_t * )
        WebPSafeMalloc(mem_size, sizeof(*mem));
        if (mem == NULL) {
    ...
    enc->argb_ = mem;
    mem = (uint32_t * )
    WEBP_ALIGN(mem + image_size);
    enc->argb_scratch_ = mem;
    mem = (uint32_t * )
    WEBP_ALIGN(mem + argb_scratch_size);
...
SearchColorGreedy(const uint32_t palette[],
                  int palette_size,
                  uint32_t color) {
    (void) palette_size;
